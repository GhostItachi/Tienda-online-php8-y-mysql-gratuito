  ...
  botonwhatsapp.addEventListener( "click", function ( e )
  {
    ...
    } ).then( ( result ) =>
    {
      if ( result.isConfirmed ) {
        window.open(
          `https://api.whatsapp.com/send?phone=${ whatsapp_negocio.value }&text=Lista: ${ productosWhatsapp } === (${ productosTotal })`,
          "_blank"
        );
      }
    } );
  ...
  fetch( ruta + "principal/listaProductos", {
    ...
    .then( ( data ) =>
    {
      ...
      if ( data.productos.length > 0 ) {
        ...
        {
          productosWhatsapp += `(${ producto.cantidad + " x " + producto.nombre + " = " + producto.precio
            }) `;
...
{
  const inputsCantidad = document.querySelectorAll( ".pro-qty input" );
  ...
  inputsCantidad.forEach( ( input ) =>
  {
    input.addEventListener( "change", function ()
    {
      ...
      Swal.fire( {
        title: "¿Estás seguro de cambiar la cantidad?",
        icon: "question",
        showCancelButton: true,
        focusConfirm: false,
        confirmButtonText: "Sí",
      ...
      } ).then( ( result ) =>
      {
        if ( result.isConfirmed ) {
          ...
          localStorage.setItem( "productos-en-carrito", JSON.stringify( productos ) );
          mostrarProductos();
          alerta( "Cantidad actualizada", 1 );
        } else {
          this.value = productos[ index ].cantidad;
        }
      } );
    } );
  } );
